...
public class Solution {
    public int getMaxRepetitions(String s1, int n1, String s2, int n2) {
        ...
        boolean flag = false;
        while(i < l1*n1){
            if (cs1[i%l1]==cs2[j%l2]){
                int s1_cnt = (i+1)/l1,s2_cnt = (j+1)/l2;
                j++;
            }
            i++;
        }
//        if(flag){
//            int currN1 = (i+1)/l1,currN2 = (j+1)/l2;
//            int currRes = currN1/currN2;
//            int moreN1 = n1%currN1,moreN2 = n2%currN2+currN1%currN2;
//            int moreRes = 0;
//            if (moreN2>0){
//                moreRes = getMaxRepetitions(s1,moreN1,s2,moreN2);
//            }
//            return currRes+moreRes;
//        }
        return j/(l2*n2);
    }
    ...
    public static void main(String[] args) {
        ...
        String res = Arrays.stream(s.split(" ")).map(String::toLowerCase).map(a -> a.length() > 2?Character.toUpperCase(a.charAt(0))+a.substring(1):a).collect(Collectors.joining(" "));
